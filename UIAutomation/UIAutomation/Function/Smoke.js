...
/**
test("[1961795] Verify click on 'goods collection' to be able to enter the Itempage in any of the goods." , function () {
    target.logDeviceInfo();
    Action.determineTheLoginWhenShopping();
    $.delay(10);
    ...
    version = version.substring(0, version.lastIndexOf("."));
    if(version == "6.1") {
        Action.tapFavoritesIcon(6);
        $.delay(sleep);
    }
    else{
        //add favorites the second goods
        Action.tapFavoritesIcon(2)
    }
    ...
    //tap goods on collection page
    Action.chooseItemOnCollectionViews(0,0,1);
    $.delay(10);

    obj.scrollDowns(1);
    $.delay(5);

    //check go to item page
    var version = target.systemVersion();
    version = version.substring(0, version.lastIndexOf("."));
    if(version == "6.1") {
    ...
    //Tap any item link,check the app
    var version = target.systemVersion();
    version = version.substring(0, version.lastIndexOf("."));
    if(version == "6.1") {
    ...
    if(version == "6.1") {
        $.delay(10);
        obj.scrollDowns(1);
        Action.tapChooseOnItemPage(2);
    }
    else{
        Action.tapChooseOnItemPage("付款及交貨方式");
    }
    $.delay(3);

    //check go to "付款及交貨方式" page
    Assert.checkSearchPage("付款及交貨方式");

    $.delay(sleep);
    var version = target.systemVersion();
    version = version.substring(0, version.lastIndexOf("."));
    if(version == "6.1") {
        Assert.pageExistText(1,"交貨方式");
        Assert.pageExistText(0,"付款方式");
    }
    else{
        Assert.pageExistText(0,"交貨方式");
        Assert.pageExistText(1,"付款方式");
    }
    Action.goBack();
    ...
    //check go to "購物須知" page
    Assert.checkSearchPage("購物須知");
    Assert.ShoppingInformationPage();
    Action.goBack();
