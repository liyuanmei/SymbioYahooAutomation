 ...
 * .android.ecstore.test.test5.Smoke.Smoke -w com.yahoo.mobile.client.android
...
public class Smoke extends ActivityInstrumentationTestCase2<Activity> {
	...
	public final void testUsePersonalization() throws Exception {
		...
		if (android.os.Build.VERSION.RELEASE.matches("4.0." + "[0-9]+")) {
			...
			for (int i = 0; i < lv.getCount(); i++) {
				View category = (View) solo.getView(
						"category_editor_grid_button", i);
			...
			assertTrue("Action bar title is wrong.", titleBar.getText()
					.toString().trim().equals(ValidationText.NEWS));
			...
			solo.clickOnView(solo
					.getView("profile_bt_edit_favorite_categories"));
			...
			assertTrue("Action bar title is wrong.", titleBars.getText()
					.toString().trim().equals(ValidationText.NEWS));
		} else {
	...
	public final void testUserCanUseWebviews() throws Exception {
		...
		if (android.os.Build.VERSION.RELEASE.matches("4.0." + "[0-9]+")) {
			...
			//Judge the web view whether is open.
			try {
				View webpage = (View) solo.getView("webpage", 0);
				assertTrue("This page incorrect.", webpage.isShown());
			} catch (AssertionFailedError e) {
