 ...
 * mobile.client.android.ecstore.test.test4.FavoriteStore.FavoriteStore -w com.yahoo
...
public class FavoriteStore extends ActivityInstrumentationTestCase2<Activity> {
	...
	public final void testFavoriteStoreLoginAndLogout() throws Exception {
		...
		try {
			...
			noResults = (TextView) solo.getView("no_result_text", 1);
	...
	public final void testNoFavoriteStoreWhenLogin() throws Exception {
		...
		if (android.os.Build.VERSION.RELEASE.matches("4.0." + "[0-9]+")) {
				...
				if (storeName.isShown()) {
					for (int i = 0; i <= 10; i++) {
						...
						try {
							...
							assertTrue("Not fully deleted.",
									solo.searchText(ValidationText.EMPTY));

						}

					}
				} else {
					junit.framework.Assert.assertTrue("Not fully deleted.",
							solo.searchText(ValidationText.EMPTY));
				}
			...
			assertTrue("Favorite store list not empty.",
					solo.searchText(ValidationText.EMPTY));
		} else {
	...
	public final void testRestricted() throws Exception {
		...
		if (solo.waitForText(ValidationText.HAS_REMOVED_COLLECTION, 1,
	 ...
	 *             if has error
	 */
	public final void testNumberOfStoreAndCollectedItems() throws Exception {
		...
		assertTrue("The number of store items,collected number"
				+ " displayed incorrectly.", productCounts && collectedCounts);
	 ...
	 *             if has error
	 */
	public final void testEnterToPromotionItems() throws Exception {
		...
		View promotion = (View) solo.getView("productitem_promotion_name",
				Action.VIEW_ID_ZERO);
	 ...
	 *             if has error
	 */
	public final void testFavoriteStoreRefresh() throws Exception {
	 ...
	 *             if has error
	 */
	public final void testFavoriteStoreAllRefresh() throws Exception {
